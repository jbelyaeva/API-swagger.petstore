...
public class putPetTests extends ApplicationManager {
  ...
  public void testPutPet() {
    ...
    petApiService().putPet(petUpdate).shouldHave(statusCode(200));

    //проверка, что данные питомцы обновлены
    Pet searchPet = petApiService().getPetById(String.valueOf(id))
        .shouldHave(statusCode(200))
        .asPojo(Pet.class);
    check().equalPetUpdateAndPetSearch(petUpdate, searchPet);
  }
}
